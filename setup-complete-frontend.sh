...
# UCab Frontend - Complete Setup Script
# This creates ALL files needed for the frontend

set -e

echo "╔═══════════════════════════════════════════════════════╗"
echo "║                                                       ║"
echo "║   🚗 UCab Frontend - Complete Setup                 ║"
echo "║                                                       ║"
echo "╚═══════════════════════════════════════════════════════╝"
echo ""

# Navigate to frontend directory
cd ~/Downloads/Cab-Booking/frontend || { echo "Error: frontend directory not found"; exit 1; }
...
<html lang="en">
  <head>
    ...
    <meta name="description" content="UCab - Your reliable cab booking service" />
    <title>UCab - Book Your Ride</title>
...
@layer components {
  .btn-primary {
    @apply bg-primary-600 hover:bg-primary-700 text-white font-medium px-6 py-3 rounded-xl 
           transition-all duration-200 hover:shadow-lg active:scale-95 disabled:opacity-50 
           disabled:cursor-not-allowed;
  }

  .btn-secondary {
    @apply bg-white hover:bg-gray-50 text-gray-900 font-medium px-6 py-3 rounded-xl 
           border border-gray-200 transition-all duration-200 hover:shadow-md active:scale-95;
  }
...
# ==================== src/App.jsx (COMPLETE APPLICATION) ====================
cat > src/App.jsx << 'APPEOF'
import React, { useState, useEffect, createContext, useContext } from 'react';
import { BrowserRouter, Routes, Route, Navigate, Link, useNavigate, useLocation } from 'react-router-dom';
import { Toaster, toast } from 'react-hot-toast';
import { Car, User, MapPin, CreditCard, Star, Clock, Menu, X, LogOut, Phone, Mail, Lock, Users, TrendingUp, DollarSign, Navigation, CheckCircle } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-600 via-primary-700 to-primary-800">
      ...
      <div className="container mx-auto px-6 py-20">
        ...
        <div className="grid md:grid-cols-3 gap-8 mt-20 max-w-5xl mx-auto">
          {[
            { icon: MapPin, title: 'Real-Time Tracking', desc: 'Track your ride in real-time' },
  ...
  const estimateFare = async () => {
    ...
    try {
      const { data } = await api.post('/rides/estimate', {
        pickupLocation: {
          coordinates: [bookingData.pickupLng, bookingData.pickupLat],
          address: bookingData.pickupAddress
        },
        dropLocation: {
          coordinates: [bookingData.dropLng, bookingData.dropLat],
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        {activeTab === 'book' && (
          <div className="grid md:grid-cols-2 gap-8">
            <Card>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-4">
                  <Button onClick={estimateFare} variant="outline" className="flex-1" disabled={loading}>
                    Estimate Fare
                  </Button>
                  <Button onClick={bookRide} className="flex-1" disabled={loading || !fare}>
                    Book Now
                  </Button>
                </div>
              </div>
            ...
            {fare && (
              <Card>
                <h3 className="text-xl font-semibold mb-6">Fare Estimate</h3>
                <div className="space-y-4">
                  <div className="flex justify-between items-center py-2 border-b">
                    <span className="text-gray-600">Distance</span>
                    <span className="font-semibold">{fare.distance} km</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b">
                    <span className="text-gray-600">Duration</span>
                    <span className="font-semibold">{fare.duration} min</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b">
                    <span className="text-gray-600">Base Fare</span>
                    <span className="font-semibold">₹{fare.fare.baseFare}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b">
                    <span className="text-gray-600">Distance Fare</span>
                    <span className="font-semibold">₹{fare.fare.distanceFare}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b">
                    <span className="text-gray-600">Tax</span>
                    <span className="font-semibold">₹{fare.fare.tax}</span>
                  </div>
                  <div className="flex justify-between items-center py-4 bg-primary-50 px-4 rounded-xl">
                    <span className="text-lg font-semibold text-primary-900">Total</span>
                    <span className="text-2xl font-bold text-primary-600">₹{fare.fare.total}</span>
                  </div>
                </div>
              </Card>
            )}
...
export default App;
APPEOF
echo "✅ Created src/App.jsx (COMPLETE APPLICATION)"
...
echo ""
echo "🎉 Your UCab frontend is ready!"
echo ""
APPEOF
chmod +x /mnt/user-data/outputs/setup-complete-frontend.sh
